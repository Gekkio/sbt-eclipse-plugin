...
public class SbtImportWizard extends Wizard implements IImportWizard {
	...
	public void addPages() {
		...
		for (IProjectNatureDescriptor natureDescriptor : workspace.getNatureDescriptors()) {
			if (natureDescriptor.getNatureId().equals(Constants.SCALA_NATURE_ID)) {
				scalaAvailable = true;
			} else if (natureDescriptor.getNatureId().equals(Constants.WST_MODULECORE_NATURE_ID)) {
				wstAvailable = true;
			}
		}
		page = new SbtImportWizardPage("Import SBT project", scalaAvailable, wstAvailable);
		addPage(page);
	}
